...
<html lang="en">
<head>
<title>Excel Mobiles || View Orders Details</title>
...
$oid = $_GET['orderid'];
$stmt = $con->prepare("SELECT * FROM payment WHERE OrderNumber = ?");
$stmt->bind_param("s", $oid);  // "s" means the parameter is a string
$stmt->execute();
$result = $stmt->get_result();
$num = $result->num_rows;

if ($num > 0) {
    // Output the payment records
    while ($payment = $result->fetch_assoc()) {
        $createdAtTime = date("H:i:s", strtotime($payment['CreatedAt']));
        ?>
        <tr>
            ...
            <td style="text-align: center; vertical-align: middle;">
                  <?php
                  $screenshot = $payment['ScreenshotPath']; // e.g., 'assets/img/payments/xyz.jpg'
                  $webPath = '/excelmobiles/excelmobiles/' . $screenshot;
                  echo '<img src="' . htmlspecialchars($webPath) . '" alt="Screenshot" style="max-width: 100%; height: auto; display: block; margin: 0 auto; border: 1px solid #ccc; padding: 4px;">';
                  ?>
              </td>


            <td><?php echo $createdAtTime; ?></td>
        </tr>
        <?php
    }
} else {
